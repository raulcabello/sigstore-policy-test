...
use kubewarden::host_capabilities::verification::{VerificationConfigV1, verify_image};
...
fn validate(payload: &[u8]) -> CallResult {
    ...
    match serde_json::from_value::<apicore::Pod>(validation_request.request.object) {
        Ok(pod) => {
            ...
            let result = verify_image("a", VerificationConfigV1{ all_of: None, any_of: None });
            match result {
                Ok(_) => {
                    kubewarden::accept_request()
                }
                Err(e) => {kubewarden::reject_request( Some(format!("pod is not accepted: {}", e)),
                                                       None)
                }
            }
